...
full_data_merge_15 <- merge(data_adults_moves, data_youth_merged_15, by = c('pidp', 'birthy', 'sex'))
...
```{r}
full_data_merge_11 <- full_data_merge_11[!is.na(full_data_merge_11$ypmkfrn), ]

full_data_merge_11$ypmkfrn_ct <- rank(full_data_merge_11$ypmkfrn)/length(full_data_merge_11$ypmkfrn)
full_data_merge_11$yppals_ct <- rank(full_data_merge_11$yppals)/length(full_data_merge_11$yppals)

stargazer(felm(moved_total ~ ypmkfrn_ct*fihhyl + sex | gor | 0 | gor, data = data_youth_merged),
...
```{r}
stargazer(lm(moved_total ~ factor(ypnpal), data = full_data_merge_15),
          lm(moved_total ~ factor(ypfpark), data = full_data_merge_15),
          lm(moved_total ~ factor(ypmkfrn)*factor(yppalo) + sex, data = full_data_merge_15),
          lm(moved_total ~ factor(yppalo), data = full_data_merge_15),
          lm(moved_total ~ factor(yppals), data = full_data_merge_15),
          omit.stat = c("adj.rsq","ser", "f"),
          omit.table.layout = c("n"),
          type = 'text')
